...
impl<'g, K: Ord, V> Cursor<'g, K, V> {
    ...
    fn pop(&mut self) -> Result<(), ()> {
        self.current.deref().inner.value.atomic_write(None)
        return Ok(())
    }
    ...
    Option<V>: AtomicRW,
{
    ...
    fn insert<'a>(&'a self, key: &'a K, value: V, guard: &'a Guard) -> Result<(), V> {
        // (*self.root).key
        // let root_cursor = self.cursor(guard);
        ...
        //     }),
        // });
        // if (*root_cursor.guard).inner.right != Atomic::null() {

        // } else {
        //     //tree is empty 
        //     //TODO add concurrency handling
        //     // self.root.load(Ordering::Relaxed, guard).deref().inner.right = new_node;
        //     return Ok(());
        // }
        // print!("{}", self.root.guard.deref().inner.value);
        // return Ok(())
    }
